...
### The Model
In `analytical.py` the Black-Scholes analytical pricing equations: $$\begin{gathered}C = SN(d_1)-Ke^{-rT}N(d_2)\\P =Ke^{-rT}N(-d_2) - SN(-d_1)\\d_1 = \frac{\ln\left(\frac{S}{K}\right)+T\left(r+\frac{\sigma^2}{2}\right)}{\sigma\sqrt{T}}, \quad \quad d_2 = d_1-\sigma \sqrt{T}\end{gathered}$$ 
...
#### Put-Call Parity
The pricing equations have an interesting relationship: $$C + Ke^{-rT} = P + S\implies C-P = S-Ke^{-rT}$$Using this equation we can verify the implementation of the equations to ensure their accuracy. This has been done across a wide range of values in `analytical_pricer.ipynb` to ensure the equations are correct. 
### Results
